export function AuthButton({ name, submitting }) {
  return (
    <button
      ...
      className={`${
        submitting ? " gap-2 pl-4" : ""
      } mx-2 flex h-[54px] w-full items-center justify-evenly  self-center rounded-lg border-[1px] border-solid bg-white  p-2 text-lg text-gray-700 shadow-sm shadow-gray-100  transition-all hover:text-gray-900 aria-disabled:border-gray-300 aria-disabled:text-gray-300 dark:bg-slate-100 dark:text-slate-800 dark:aria-disabled:text-gray-300`}
      type="submit"
    >
      {name}
      {submitting ? (
